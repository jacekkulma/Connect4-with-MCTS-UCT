...
seeds = [
    ...
    67890, 101010, 111111, 121212, 98765
]

def main(skip_self_comparison=True) -> None:
    # Skip comparing the model with itself, if the flag is set
    if skip_self_comparison:
        models_to_compare = combinations(models, 2)
    else:
        models_to_compare = combinations_with_replacement(models, 2)
    ...
    for model1, model2 in models_to_compare:
        ...
        file_name = f"{model1.__name__}-vs-{model2.__name__}.csv"
